...
/* ── 1. Engine boot ────────────────────────────────────── */
KSS_ENGINE.init({
  serviceId: "695e6657535fb973f32ec59e",
  type: "menu",
  filterEmpty: true,
...
KSS_ENGINE.error.subscribe(function (err) {
  if (!err) return;
  var overlay = document.getElementById("progress-bar");
  if (overlay) {
    var text = overlay.querySelector("#loader-logo");
    var progress = overlay.querySelector("#progress");
...
KSS_ENGINE.onReady(function (menuData) {
  ...
  if (loaderLogo) {
    loaderLogo.textContent = STORE.storeName;
  }
  ...
  KSS_ENGINE.waitForImages().then(function () {
    ...
    // Init search & features (loaded as separate scripts)
...
/* ── DOM refs ──────────────────────────────────────────── */
var menuBtnDOM = document.getElementById("btn-group");
var menuBodyDOM = document.getElementById("menu-body");
var menuHeader = document.getElementById("header");
var content = document.getElementById("content");
var btnDom = document.querySelector("#btn-group");
...
function buildHeader(STORE) {
  menuHeader.innerHTML =
    ...
    (STORE.sm && STORE.sm.locationUrl
      ? '<div class="details"><a class="a-white" href="' +
        STORE.sm.locationUrl +
        '" target="_blank" rel="noopener noreferrer" aria-label="Open location in maps">' +
    ...
    (STORE.sm && STORE.sm.instagramUrl
      ? '<div class="details"><a href="' +
        STORE.sm.instagramUrl +
        '" target="_blank" rel="noopener noreferrer" aria-label="Open Instagram">' +
    ...
    (STORE.sm && STORE.sm.facebookUrl
      ? '<div class="details"><a href="' +
        STORE.sm.facebookUrl +
        '" target="_blank" rel="noopener noreferrer" aria-label="Open Facebook">' +
        '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-facebook" viewBox="0 0 16 16"><path d="M16 8.049c0-4.446-3.582-8.05-8-8.05C3.58 0-.002 3.603-.002 8.05c0 4.017 2.926 7.347 6.75 7.951v-5.625h-2.03V8.05H6.75V6.275c0-2.017 1.195-3.131 3.022-3.131.876 0 1.791.157 1.791.157v1.98h-1.009c-.993 0-1.303.621-1.303 1.258v1.51h2.218l-.354 2.326H9.25V16c3.824-.604 6.75-3.934 6.75-7.951"/></svg>' +
        STORE.storeName +
        "</a></div>"
      : "") +
    (STORE.phoneNumber
      ? '<div class="details"><a href="tel:' +
        STORE.phoneNumber +
        '" class="a-white" aria-label="Call restaurant">' +
        '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-telephone-fill" viewBox="0 0 16 16"><path fill-rule="evenodd" d="M1.885.511a1.745 1.745 0 0 1 2.61.163L6.29 2.98c.329.423.445.974.315 1.494l-.547 2.19a.68.68 0 0 0 .178.643l2.457 2.457a.68.68 0 0 0 .644.178l2.189-.547a1.75 1.75 0 0 1 1.494.315l2.306 1.794c.829.645.905 1.87.163 2.611l-1.034 1.034c-.74.74-1.846 1.065-2.877.702a18.6 18.6 0 0 1-7.01-4.42 18.6 18.6 0 0 1-4.42-7.009c-.362-1.03-.037-2.137.703-2.877z"/></svg>' +
        (STORE.displayedPhoneNumber || STORE.phoneNumber) +
        "</a></div>"
      : "");
}
...
function displayMenu(subCategories, STORE) {
  ...
  subCategories.forEach(function (sub) {
    ...
    section.setAttribute("aria-labelledby", sub._id + "-title");
    section.innerHTML =
      '<div class="section-header">' +
      (sub.bgImg
        ? '<div class="section-img-container">' +
          '<div class="section-img-bg" style="background-image: url(\'' +
          sub.bgImg +
          "')\"></div>" +
          '<img src="' +
          sub.bgImg +
          '" alt="' +
          sub.label +
          '" class="section-img-main" tabindex="0" role="button" aria-label="View ' +
          sub.label +
          ' image"/></div>'
        : "") +
      '<p id="' +
      sub._id +
      '-title">' +
      KSS_ENGINE.capitalizeFirstLetter(sub.label) +
      "</p></div>";
    ...
    sub.items.forEach(function (item) {
      ...
      var itemImg = item.img
        ? '<div class="item-img-container" data-src="' +
          item.img +
          '">' +
          '<div class="item-img-bg"></div>' +
          '<img class="item-img-main" alt="' +
          item.label +
          '" loading="lazy" /></div>'
        : "";
...
function setupScrollLinks() {
  ...
  scrollLinks.forEach(function (link) {
    link.addEventListener("click", function (e) {
      ...
      var navHeight = btnDom.getBoundingClientRect().height;
...
function setupIntersectionObserver() {
  ...
  function setActiveButton(id) {
    buttons.forEach((btn) => {
      const isActive = !!btn.querySelector(`a[href="#${id}"]`);
...
/* ==========================================================
   SCROLL / UP BUTTON / WHATSAPP VISIBILITY
   ========================================================== */
var upBtn = document.getElementById("up-btn");
var progressRingFill = document.querySelector("#up-btn .progress-ring-fill");
var circumference = 2 * Math.PI * 22.5;

function updateScrollIndicator() {
  var scrollPosition = window.scrollY;
  var documentHeight =
    document.documentElement.scrollHeight - window.innerHeight;
  var scrollPercentage = Math.min(scrollPosition / documentHeight, 1);
  var offset = circumference * (1 - scrollPercentage);
...
function updateStickyNavState() {
  ...
  btnDom.classList.toggle(
    "is-stuck",
    window.scrollY > 0 && btnDom.getBoundingClientRect().top <= 0,
  );
}
...
document.addEventListener("keydown", function (event) {
  ...
  if ((event.key === "Enter" || event.key === " ") && event.target === wpBtn) {
    event.preventDefault();
    var link = wpBtn.querySelector("a");
    link && link.click();
  }
});
